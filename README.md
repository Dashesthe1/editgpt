...
M5 semantic-event performance memoizes exact-frame Qwen judgments by source fingerprint, normalized description, and frame index, so repeated queries in the long-lived analyzer do not resend already-classified frames. PyAV sources now also support timestamp-guided exact-frame seeking from the temporal profile: requested frames are mapped back to the profile's decoded timestamps, verified within a sub-frame tolerance, and conservatively fall back to full decode if verification fails. Semantic search keeps the correctness-preserving 384px evidence path, uses compact per-frame classifications, and switches from a 12-frame coarse batch to 4-frame frontier batches after the first negative pass.

On the same 24-second Spider-Man proof, the accepted path preserves frame 683 / 11.3833 s with bracket 682-683. The semantic stage fell from about 61.3 s originally, to 49.1 s after memoization, to roughly 14.7-21.9 s in current warm-model measurements; an identical cached repeat remains about 0.004-0.005 s. Timestamp-guided retrieval around the target measures roughly 0.28-0.38 s for clustered exact-frame reads. The earlier 320px experiment remains rejected because it misclassified suit-detail close-ups as full-person visibility. Full temporal-profile construction is now the dominant first-use cost and is the next optimization target.
