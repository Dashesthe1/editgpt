...
M5 semantic-event performance now memoizes exact-frame Qwen judgments by source fingerprint, normalized description, and frame index, so repeated queries in the long-lived analyzer do not resend already-classified frames. On the same 24-second Spider-Man proof, the correctness-preserving 384px path reduced the cold semantic appearance search from about 61.3 s to 49.1 s and an identical warm repeat to about 0.005 s, while preserving frame 683 / 11.3833 s with bracket 682-683. A 320px experiment reached about 20.1 s cold but misclassified suit-detail close-ups as full-person visibility, so that lower-resolution path was rejected rather than trading semantic correctness for speed. Cold first-query latency remains an explicit optimization target.
