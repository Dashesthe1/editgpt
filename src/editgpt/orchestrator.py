        ...
        if name == "mcp":
            ...
            for start in (self.start_eyes_mcp(), self.start_hands_mcp()):
                if not start.get("ok"):
                    print(json.dumps(start, indent=2))
                    return 6
        elif name in {"semantic-pointer", "semantic-click", "hands-ui", "controller"}:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    proof.add_argument("name", choices=("capture", "mcp", "hands", "loop", "semantic-pointer", "semantic-click", "hands-ui", "controller", "semantic"))
